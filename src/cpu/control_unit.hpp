    ...
    bool execute_cycle(Memory& memory, GPRs& gprs, SPRs& sprs, BusSystem& buses) {
        ...
        switch (instr.opcode) {
            ...
            case Opcode::NOP: {
                if (instr.is_call()) {
                    // CALL RS1
                    // 1. Push return address (PC + 2) onto the stack
                    uint16_t return_addr = sprs.PC + 2;
                    sprs.SP -= 2;                          // grow stack downward
                    memory.write_word(sprs.SP, return_addr);

                    // 2. Jump to target address in RS1
                    uint16_t target = static_cast<uint16_t>(gprs[instr.rs1]);
                    sprs.PC     = target;
                    pc_updated  = true;

                    if (trace_enabled) {
                        std::cout << "[EXECUTE] CALL -> pushed return addr 0x"
                                  << std::hex << return_addr
                                  << " to SP=0x" << sprs.SP
                                  << ", jumping to 0x" << target
                                  << std::dec << std::endl;
                    }
                } else if (instr.is_ret()) {
                    // RET
                    // 1. Pop return address from stack
                    uint16_t return_addr = memory.read_word(sprs.SP);
                    sprs.SP += 2;                          // shrink stack
                    ...
                    if (trace_enabled) {
                        std::cout << "[EXECUTE] RET -> popped return addr 0x"
                                  << std::hex << return_addr
                                  << ", SP=0x" << sprs.SP
                                  << std::dec << std::endl;
                    }
            ...
            case Opcode::XOR: {
                ...
                int16_t val2 = instr.is_immediate ? static_cast<int16_t>(instr.imm)
                                                  : gprs[instr.rs2];

                ALUResult result;
                switch (instr.opcode) {
                    case Opcode::ADD: result = ALU::add(val1, val2);       break;
                    case Opcode::SUB: result = ALU::subtract(val1, val2);  break;
                    case Opcode::AND: result = ALU::and_op(val1, val2);    break;
                    case Opcode::OR:  result = ALU::or_op(val1, val2);     break;
                    case Opcode::XOR: result = ALU::xor_op(val1, val2);    break;
                    default: break;
                }
            ...
            case Opcode::SHR: {
                ...
                ALUResult result = (instr.opcode == Opcode::SHL)
                                 ? ALU::shift_left(val, shift)
                                 : ALU::shift_right(val, shift);
            ...
            // ── Jumps ─────────────────────────────────────────────────────────
            case Opcode::JMP: {
                uint16_t base   = (gprs[instr.rs1] == 0) ? (sprs.PC + 2)
                                                          :  gprs[instr.rs1];
        ...
        // Advance PC if not modified by a jump/call/ret
        if (!pc_updated) {
            sprs.PC += 2;
        }
